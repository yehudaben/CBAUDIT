...
Seven steps: version strings agree, app-state dump, field capture (11,284
...
behaviour. **Zero mismatches is the only pass** — a step can exit 0 and still
print mismatch counts, so read the numbers.
...
Nothing is shared between users and nothing reaches a server:
...
- **`$ RDR` is captured but unused** — $207,358.82 across 153 MIDs on the Aug 14
  export, which exceeds the total CB volume of $140,719.52. A dollar-deflection
  view is the obvious next feature.
